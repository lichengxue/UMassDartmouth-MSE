...
setwd("C:/Users/swulfing/Documents/GitHub/UMassDartmouth-MSE/SW")                                 # <- ADJUST if needed
...
# om_ecov$rep$N1
## 8.4 Bring over historical index usage and sigma (rows 1:50) then extend flat ##### CHANGED HERE #######
om_input_P$data$agg_index_sigma[1:50, ] <- input_hist$data$agg_index_sigma[1:50, ]
...
####
scenarios$no_ecov = scenarios$HistAvg
scenarios$no_ecov$recruitment_how[1] = "none"
set.seed(123 + 1L)
mod3 <- loop_through_fn(
  ...
  sel_em = sel_em,
  NAA_re_em = NAA_re_em,
  ecov_em = scenarios$no_ecov,                   # <- EM will estimate Ecov state, but no link between ecov and rec
  ecov_em_opts = list(use_ecov_em = TRUE, period = period_rows, lag = 1),
  age_comp_em = age_comp_em,
  em.opt = list(separate.em = FALSE, separate.em.type = 1, do.move = FALSE, est.move = FALSE),
  update_index_info = update_index_info_list,
...
scenarios$terminal_ecov = scenarios$HistAvg
scenarios$terminal_ecov$mean[52:57,] = scenarios$terminal_ecov$mean[51,]
set.seed(123 + 1L)
mod4 <- loop_through_fn(
  ...
  sel_em = sel_em,
  NAA_re_em = NAA_re_em,
  ecov_em = scenarios$terminal_ecov,                   # <- EM will estimte Ecov state, but no link between ecov and rec
  ecov_em_opts = list(use_ecov_em = TRUE, period = period_rows, lag = 1),
  age_comp_em = age_comp_em,
  em.opt = list(separate.em = FALSE, separate.em.type = 1, do.move = FALSE, est.move = FALSE),
  update_index_info = update_index_info_list,
...
# If you want to compare SSB performance between 4 management strategies (only feedback years)
plot(mod1$om$rep$SSB[53:56,], type = "l", lwd = 2, main = "SSB")           # black
lines(mod2$om$rep$SSB[53:56,], col = "red",   lwd = 2)
lines(mod3$om$rep$SSB[53:56,], col = "blue",  lwd = 2)
lines(mod4$om$rep$SSB[53:56,], col = "green", lwd = 2)

legend("topleft",
       legend = c("AR1","Average","No_Ecov","Last_Ecov"), # x axis is years of mse
...
plot(mod1$om$rep$pred_catch[53:56,], type = "l", lwd = 2, main = "Catch")           # black
lines(mod2$om$rep$pred_catch[53:56,], col = "red",   lwd = 2)
lines(mod3$om$rep$pred_catch[53:56,], col = "blue",  lwd = 2)
lines(mod4$om$rep$pred_catch[53:56,], col = "green", lwd = 2)
...
# THIS IS AGE SPECIFIC F. THIS CASE OF THE OLDEST AGE GROUP
plot(mod1$om$rep$Fbar[53:56,1], type = "l", lwd = 2, main = "Fbar")           # black
lines(mod2$om$rep$Fbar[53:56,1], col = "red",   lwd = 2)
lines(mod3$om$rep$Fbar[53:56,1], col = "blue",  lwd = 2)
lines(mod4$om$rep$Fbar[53:56,1], col = "green", lwd = 2)
...
# OVERFISHING STATUS OVER TIME WANT BELOW ONE (F<FMSY)
plot(mod1$om$rep$Fbar[,1]/exp(mod1$om$rep$log_FMSY_static))
#.2 IN MOST RECENT YEAR NOT GREAT, WE WANT TO FISH CLOSE TO 1. WE'RE USING MSY PROXY OF F.40
# OVER FISHED STATUS OVER TIME WANT ABOVE ONE SSB>SSBFMSY
plot(mod1$om$rep$SSB/exp(mod1$om$rep$log_SSB_FXSPR_static))

# THIS DOESN'T USE MSY AS PROXY. SIMLAR TO MANAGEMENT
plot(mod1$om$rep$Fbar[,1]/exp(mod1$om$rep$log_Fbar_XSPR_static)[1])
plot(mod1$om$rep$SSB/exp(mod1$om$rep$log_SSB_FXSPR_static[1]))


mod1$om$rep # list of pop and catch info of REAL dynamics
# Compare. These should be VERY similar in historical section
mod1$om$rep$SSB - mod2$om$rep$SSB
mod1$om$rep$Fbar[,1] - mod2$om$rep$Fbar[,1]
...
# Param ests from EM (depends on how many EMs in feedback)
mod1$par.est[[1]]# first EM param ests
mod1$par.est[[1]]$mean_rec_pars # I think this is in log scale
mod1$par.est[[1]]$Ecov_process_pars # ALL IN LOG STATE. NEED TO TRANSFORM BACK. HE GIVES THAT CODE ABOVEmean, error dist SD, then logit_rho
...
mod1$om$parList$mean_rec_pars # Real stock recruit relationship
mod1$par.est[[1]]$mean_rec_pars # mean recruit in log scale. Over estimating recruitment
