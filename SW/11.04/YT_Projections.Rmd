...
ecov_om <- list(
  label = "bt_temp",
  mean  = as.matrix(c(env.dat_me$bt_temp, rep(NA_real_, 51))),   # Historical temperature data
  logsigma = "est_1",                                            # Obs standard errors. 
  ...
  process_model = "ar1",                                         # 1st order Auto-Regressive process model
  recruitment_how = matrix("controlling-lag-1-linear")           # how each ecov affects recruitment for each stock. 
  
  
  
  
 # Obs standard errors. Estimated value for each time step as random effects with two parameters (mean, var) 
#   "controlling"
...
EMtempPlot <- ggplot(data.frame(ecov_om), aes(x = year, y = mean)) +
  ...
  geom_line(aes(color = "OM")) +
  scale_color_manual(name='Model',
                     breaks=c('AR1', 'RecWind', 'HistAvg', 'RecTrend', 'TermYear', 'OM'),
                     values=c( 
                              'AR1'="#CD0BBC",
                              'RecWind' = "#F5C710" ,
                              'HistAvg' =  "#3283FE",
