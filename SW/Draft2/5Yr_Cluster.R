...
assess.interval <- 5L                                            # assess every 3 yrs
...
##    - years 1972–2073 (= 57 rows) with a 1-year lag
##    - we will build a PROJECTED trend scenario for 2023–2073
## ============================================================

## 7.2 PROJECTED trend for 2023–2073 by linear regression over historical (1972+)
## MED PROJECTION
...
for(i in 0:51){
  if (i %% 5 == 0){
    HistAvg[(i+1):(i+5)] <- mean(ecov_om_P$mean[(1:k)])
    RecWind[(i+1):(i+5)] <- mean(ecov_om_P$mean[((k-4):k)])
    ...
    RecTrend$Temp[(i+4)] <- om_RecentTrend[["coefficients"]][1] + (om_RecentTrend[["coefficients"]][2] * ecov_om_P$year[k + 3])
    RecTrend$Temp[(i+5)] <- om_RecentTrend[["coefficients"]][1] + (om_RecentTrend[["coefficients"]][2] * ecov_om_P$year[k + 4])
  }
  k <- k + 1
}
