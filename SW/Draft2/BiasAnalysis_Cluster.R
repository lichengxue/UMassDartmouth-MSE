...
model_nums <- 1:6 # CHANGE HERE Number of Projections
nsim <- c(0:99) # CHANGE HERE number of simulations/seed
ModNames <- c("Mod_AR1Ecov", "Mod_NoEcov", "Mod_LOWEcov", "Mod_MEDLOWEcov", "Mod_MEDEcov", "Mod_HIGHEcov") # CHANGE NAME HERE
...
mods <- lapply(nsim, function(r) {
  tryCatch({
    mod_list <- lapply(model_nums, function(m) {
      ...
      newr <- r + ((m -1) * 100)
      file_path <- file.path(sprintf(paste0('/work/pi_gfay_umassd_edu/Wulfing/CEFI_Draft2/Bias/OMHigh/',simLocations,"/block_%d_sim_%d_output.rds"), m, newr))
      readRDS(file_path)
      #print(file_path)
    })
