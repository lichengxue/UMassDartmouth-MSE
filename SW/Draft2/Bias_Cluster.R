...
####### changes made to script 12/14/25 KW #######
#args linking seeds to batch script 
#added block switch to the sims; removed for loops 
#changed code for saving runfiles 
#removed all plots since this had issues on my end when testing package and not necessary for now  

args <- commandArgs(trailingOnly = TRUE)

seed   <- as.numeric(args[which(args == "--seed") + 1])
sim_id <- as.numeric(args[which(args == "--sim_id") + 1])
block  <- as.numeric(args[which(args == "--block") + 1])
...
######################################
## CHANGE HERE BASED ON om SCENARIO ##
######################################
...
MSE_years     <- 51L                                             # projection/feedback years for MSE
...
ecov_om <- list(
  label = "bt_temp",
  mean  = as.matrix(c(env.dat_me$bt_temp, rep(NA_real_, 52))),   # fill future later
  logsigma = "est_1",
  year  = 1972:2073,                                            # includes 1-lag year (1972) + 1973–2028
  use_obs = matrix(1, ncol = 1, nrow = 102),                     # observed everywhere (we may override in MSE)
  process_model = "ar1",
...
sel3 <- list(
  ...
  initial_pars = list(
    c(0.05, 0.3, 0.7, 0.875, 0.9, 1),   # fleet age-specific (fix older ages below)
    c(2.313, 0.328),                    # index 1 logistic
    c(1.62, 0.483),                    # index 2 logistic
    c(2.144, 0.217)                     # index 3 logistic
  ),
  fix_pars = list(c(6), NULL, NULL, NULL) # fix ages 4–6 at 1 for fleet selectivity
)
...
proj_df <- data.frame(year_proj = 2022:2073)                    # we only need 2023–2028, but harmless to compute 2022 too
...
OMscenarios <- list(LOW = ecov_om$mean, 
                  MEDLOW = ecov_om$mean,
                  MED = ecov_om$mean,
                  HIGH = ecov_om$mean)

OMscenarios$LOW[51:102] <- proj_LOW$Temp_proj[proj_df$year_proj >= 2022]  # 51->2022, 52->2023, ..., 57->2028

OMscenarios$MEDLOW[51:102] <- proj_MEDLOW$Temp_proj[proj_df$year_proj >= 2022]  # 51->2022, 52->2023, ..., 57->202

OMscenarios$MED[51:102] <- proj_MED$Temp_proj[proj_df$year_proj >= 2022]  # 51->2022, 52->2023, ..., 57->202

OMscenarios$HIGH[51:102] <- proj_HIGH$Temp_proj[proj_df$year_proj >= 2022]  # 51->2022, 52->2023, ..., 57->202

# plotData <- data.frame(year = ecov_om$year,
#                        OM_1 = scenarios$LOW,
#                        OM_2 = scenarios$MEDLOW,
#                        OM_3 = scenarios$MED,
#                        OM_4 = scenarios$MEDLOW
# )
# 
...
ecov_om_P <- ecov_om
ecov_om_P$mean <- OMscenarios[[PICKSCENARIO]]
...
om_input_P$par$Ecov_re[52:102, 1] <- om_input_P$data$Ecov_obs[52:102, 1] - long_term_bt_mean
...
ecov_LOW <- ecov_me
ecov_LOW$mean <- OMscenarios$LOW

ecov_MEDLOW <- ecov_me
ecov_MEDLOW$mean <- OMscenarios$MEDLOW

ecov_MED <- ecov_me
ecov_MED$mean <- OMscenarios$MED

ecov_HIGH <- ecov_me
ecov_HIGH$mean <- OMscenarios$HIGH
...
cat("Simulation", sim_id, "running with seed", seed, "in block", block, "\n")
...
mod <- switch(block, 
              {
                ...
                tryCatch({
                  ...
                  outfile <- file.path(outdir, paste0("block_", block, "_sim_", sim_id, "_output.rds"))
