...
# ecov_om_P <- ecov_om
# ecov_om_P$mean[51:102, 1] <- proj_df$Temp_proj[proj_df$year_proj >= 2022]  # 51->2022, 52->2023, ..., 57->2028
# 
...
# ecov_TermYear$mean[51:102,] <- om_with_data_P$rep$Ecov_x[51]#ecov_om_P$mean[50,]
# 
# 
...
# ## ============================================================
# 
# ModelLocations <- 'C:/Users/swulfing/Documents/MSE_results/RecRETest'
# 
# # library(doParallel)
# # library(foreach)
# # 
...
for(i in 1:10){
  if(i == 1){
    ecoRecs <- data.frame(mods[[i]][["Mod1"]][["om"]][["rep"]][["Ecov_x"]][1:101,1])
    colnames(ecoRecs)[i] <- i
    
    SSB <- data.frame(mods[[i]][["Mod1"]][["om"]][["rep"]][["SSB"]])
    colnames(SSB)[i] <- i
    
    Epsilon <- data.frame(mods[[i]][["Mod1"]][["om"]][["rep"]][["NAA_devs"]][1,1,,1])
    colnames(Epsilon)[i] <- i

  }else{
    ecoRecs <- cbind(ecoRecs, mods[[i]][["Mod1"]][["om"]][["rep"]][["Ecov_x"]][1:101,1])
    SSB <- cbind(SSB, mods[[i]][["Mod1"]][["om"]][["rep"]][["SSB"]])
    Epsilon <- cbind(Epsilon, mods[[i]][["Mod1"]][["om"]][["rep"]][["NAA_devs"]][1,1,,1])
  }
...
for(i in 1:nrow(ecoRecs)){
  for(j in 1:ncol(ecoRecs)){
    R_t[i,j] <- log(SSB[i,j]) + (betas[j] * ecoRecs[i,j]) + Epsilon[i,j]
  }}

PerVar_ecov <- c()
for(i in 1:ncol(R_t)){
  PerVar_ecov[i] <- var(betas[i] * ecoRecs[,i])/var(R_t[,i])
}
