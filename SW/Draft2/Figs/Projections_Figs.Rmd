...
BRP_data <- readRDS('C:/Users/swulfing/Documents/GitHub/UMassDartmouth-MSE/SW/Draft2/Figs/Projections_BRP.rds')
...
```{r recruitmentDevs, fig.cap = 'Percent of recruitment deviationexplained by ecov in each model. NOTE WE ARE GETTING NEGATIVE SDS FOR NAA SIGMA RECRUITMENT SO RIGHT NOW IM ADDING RECRUITMENTS BUT IM SURE THAT IS WRONG. STILL LOOKING INTO IT', echo = FALSE, warning = FALSE, message = FALSE}
...
# ggplot(RecDevs, aes(x = EM_name, y = Rec_Ecov)) +
#   geom_boxplot() +
#   xlab('EM') +
...
Prop_overfishing <- No_Overfishing %>%
  group_by(EM_name) %>%
  summarize(
            total = n(),
            PropOM_Overfishing = sum(OM_Overfishing >= 1),
            PropEM_Overfishing = sum(EM_Overfishing >= 1),
            PropOM_Overfished = sum(OM_Overfished <= 1),
            PropEM_Overfished =  sum(EM_Overfished <= 1)
            )
