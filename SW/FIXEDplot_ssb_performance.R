FIXEDplot_ssb_performance <- function (mods, is.nsim, main.dir, sub.dir, var = "SSB", width = 10, 
          ...
          new_model_names = NULL, base.model = NULL) 
  {
    ...
    Years <- if (!is.nsim) 
      mods[[1]]$om$years
    else mods[[1]][[1]]$om$years
    res <- if (!is.nsim) {
      lapply(seq_along(mods), function(i) {
        data.frame(SSB = mods[[i]]$om$rep$SSB, Model = paste0("Model", 
      ...
      lapply(seq_along(mods), function(r) {
        lapply(seq_along(mods[[r]]), function(m) {
          data.frame(SSB = mods[[r]][[m]]$om$rep$SSB, Model = paste0("Model", 
                                                                     m), Year = Years, Realization = r) %>% tail(use.n.years)
        }) %>% bind_rows()
      }) %>% bind_rows()
    }
    res <- res %>% rowwise() %>% mutate(SSB_Global = sum(c_across(starts_with("SSB")), 
                                                         na.rm = TRUE)) %>% ungroup()
    ...
    if (!is.null(base.model)) {
      ...
      res <- res %>% left_join(base_df, by = c("Realization", 
    ...
    if (plot.style == "boxplot") {
      ...
      res_limits <- res %>% group_by(Model, Label) %>% summarise(min_val = min(!!sym(var), 
                                                                 ...
                                                                 .groups = "drop")
      res_summary <- res_summary %>% left_join(res_limits, 
    ...
    if (!file.exists(new_sub_dir)) 
      dir.create(new_sub_dir, recursive = TRUE)
